...
- **管理员控制台**（`/console`，登录后导航栏入口）：**Xshell 风格终端**——命令与输出在同一滚动区，回车后提示符移到下一行，提示符实时显示当前工作目录；**实时流式输出**（WebSocket，长命令/脚本边跑边显示，`\r` 进度条原地覆盖）；**Ctrl+C / ⏹ 停止**中断当前命令（Windows taskkill 杀进程树）；**Tab 补全**命令名与路径；**cd 持久工作目录**；↑/↓ 历史、Ctrl+L 清空、10 分钟超时、退出码显示；**服务端实时日志**（每 3 秒刷新）。仅管理员可用：路由守卫 401 + 服务端 `authRequired` + WebSocket token 三重校验
