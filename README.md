...
- **路由懒加载**：所有页面按需下载代码块；`three.js`（3D 二维码）、`xterm`（控制台）、TipTap 富文本编辑器等重依赖只在实际打开对应页面时加载，首屏不再一次性下载全部页面
- **顶部进度条**：页面切换 / 懒加载期间顶部显示进度动画，避免长加载时白屏或无反馈
...
- **HTML 渲染**（`/tools/html-render`）：粘贴 HTML 代码，点击「渲染」（或 Ctrl+Enter）在**新标签页**中原样显示——样式与脚本都正常执行；完整文档原样渲染，片段自动补全为最小页面骨架；内置示例。**一次性渲染**：代码不经过服务器、不写入本地存储，渲染成功后输入框立即清空、Blob URL 随即释放，新标签页关闭后不留任何副本（纯前端处理，代码不会上传）。**特殊入口**：输入 `login` 或 `inside` 后点击「渲染」会分别触发登录弹窗 / 内部人员身份，而不是渲染 HTML
...
- **访问统计**：设置页新增“访问统计”（仅管理员可见）——总访问量 / 今日 / 昨日 / 独立 IP、近 30 天访问趋势、热门页面与热门 IP、按时间倒序的访问记录（IP、IP 归属地、路径、来源页、User-Agent），支持按 IP / 路径 / UA 搜索与分页；**点击热门页面**可查看访问过该页面的 IP 列表（按访问次数排序，支持分页与继续查看 IP 详情）；**点击任意访问记录行**可查看该条完整详情（完整路径 / Referer / UA 等），点击 IP 可继续打开 **IP 来源详情**（完整归属地、ISP、经纬度、访问汇总、路径/UA 分布与最近访问记录）；并内置 **IP 来源地图**（Leaflet + OpenStreetMap），用圆点标注访问量、热力图显示热点区域。生产模式由 Express 记录整页加载，SPA 内部路由切换由前端自动上报，避免重复计数；IP 归属地使用 ip-api.com 免费接口按需异步解析并缓存到 SQLite（含经纬度）
...
- **内部人员身份**：只读的中间身份。在 **Tools → HTML 渲染** 输入 `inside` 并点击「渲染」即获取（口令见 `server/.env` 的 `INSIDER_KEYWORD`，默认 `inside`），导航栏出现「🔑 内部模式」徽标，可点 ✕ 退出；内部人员能看到游客看不到的页面与文章，但不能编辑、不能进设置页；**进入内部模式后全站页面都显示壁纸背景**（与 Anime 页共用同一套壁纸与缓存）
- **隐藏登录入口**：界面不显示任何登录按钮，在 **Tools → HTML 渲染** 输入 `login` 并点击「渲染」弹出登录框（Esc 或点击遮罩关闭）；已登录右上角显示当前站点版本号与提交 ID（格式 `版本号.提交ID`）以及 设置 / 退出 按钮
...
**内部人员口令**：`server/.env` 的 `INSIDER_KEYWORD`（默认 `inside`）为内部模式口令；在 **Tools → HTML 渲染** 输入 `inside` 后点击「渲染」即可触发。若修改该口令，需同步更新 `src/views/HtmlRenderToolView.vue` 中触发时发送的关键词（默认写死 `inside`）。
...
部署完成后访问 `https://你的域名`，在 **Tools → HTML 渲染** 输入 `login` 并点击「渲染」弹出登录框，输入管理员账号登录。日常更新代码：
