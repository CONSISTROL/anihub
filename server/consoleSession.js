...
// 支持多终端：每个 WebSocket 连接各自 createSession()，进程/目录/中断计数互不影响。
// Linux 上通过系统自带的 script（util-linux）给子进程分配伪终端（PTY），
// 从而支持 su/passwd/htop/vim 等交互命令（终端模拟器 xterm.js 自行解析 ANSI）。
// 与 REST 路由（console.js）共享默认会话（session）；WebSocket（consoleSocket.js）每连接一个会话。
...
/** REST 兼容的默认会话（控制台多终端各自用 createSession） */
export const session = createSession()

/** cd 命令：更新会话工作目录（一次性子进程无法保留 cd） */
export function tryCd(sess, cmd) {
  if (!/^cd(?:\s|$)/i.test(cmd)) return null
  let raw = cmd.slice(2).trim()
  raw = raw.replace(/^\/d\s+/i, '') // Windows cd /d
  if (!raw) return { ok: true, dir: sess.dir, msg: '' }
  const target = raw.replace(/^"(.*)"$/, '$1').trim()
  const resolved = path.resolve(sess.dir, target)
  let isDir = false
  try {
    isDir = fs.statSync(resolved).isDirectory()
  } catch {}
  if (!isDir) {
    return { ok: false, dir: sess.dir, msg: `系统找不到指定的路径: ${raw}` }
  }
  sess.dir = resolved
  return { ok: true, dir: sess.dir, msg: '' }
}
...
function resetSoftKill(sess) {
  if (sess.softKillTimer) clearTimeout(sess.softKillTimer)
  sess.softKillTimer = null
  sess.softKillCount = 0
}
 ...
 */
export function killCurrent(sess, hard = false) {
  const p = sess.proc
  if (!p || !p.pid) return false
  if (sess.pty && !hard) {
    sess.softKillCount++
    if (sess.softKillTimer) clearTimeout(sess.softKillTimer)
    sess.softKillTimer = setTimeout(() => resetSoftKill(sess), 8000)
    try {
      p.stdin?.write('\x03')
    } catch {}
    if (sess.softKillCount >= 3) {
      resetSoftKill(sess)
      return killCurrent(sess, true)
    }
    return true
  }
  try {
 ...
 */
export function startStream(sess, cmd, { onOut, onErr, onCwd, onExit, noTimeout = false }) {
  killCurrent(sess, true) // 单会话内：新命令替换旧命令（硬杀旧进程）
  const cd = tryCd(sess, cmd)
  if (cd) {
  ...
  const finish = (r) => {
    ...
    // 旧 finish 可能晚于新进程启动——不能把新进程的 sess.proc 清掉
    if (sess.proc === child) {
      sess.proc = null
      sess.pty = false
      sess.resetTimer = null
    }
    if (onExit) onExit(r)
  }
  const onTimeout = () => {
    killCurrent(sess, true)
    finish({ exitCode: 124, timedOut: true, cwd: sess.dir })
  }
  let timer = null
  const resetTimer = () => {
    if (!timer) return
  ...
  child.on('error', (e) => {
    finish({ exitCode: 1, timedOut: false, cwd: sess.dir, error: e.message })
  })
  child.on('exit', (code, signal) => {
    decOut.flush()
    decErr.flush()
    const timedOut = !!signal || code === 124
    finish({ exitCode: code ?? (timedOut ? 124 : 1), timedOut, cwd: sess.dir, signal })
  })
}
